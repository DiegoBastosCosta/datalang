...
class TopicModelingTools:
    ...
    # All the methods inside BertopicTools go here, 
    # but without the 'BertopicTools.' prefix in the method definitions. 
    # Also, replace all instances of 'BertopicTools.' inside the method bodies with 'self.'.
    ...
    def display_data(self) -> None:
      ...
      """
      docs = st.session_state[Constants.DOCS.value]
      topic_model = st.session_state[Constants.TOPIC_MODEL.value]
      topics = st.session_state[Constants.TOPICS.value]
      probs = st.session_state[Constants.PROBS.value]
      ...
      with st.expander("Topic info"):
          ...
          if ti_col2.button('Question'):
              ...
              st.session_state[Constants.LANGCHAIN_HISTORY.value]['topic_info'].append({'query':query,'response':response})
          for qa in st.session_state[Constants.LANGCHAIN_HISTORY.value]['topic_info']:
...
class TopicModelingUI:
    def __init__(self, topic_modeling_tools: TopicModelingTools, homepage_processor: HomePageProcessor):
        self.topic_modeling_tools = topic_modeling_tools
        self.homepage_processor = homepage_processor
        # Initialize the Streamlit session state here if necessary

    # The 'render' method of BertopicTools goes here, 
    # but you need to replace 'BertopicTools.' with 'self.topic_modeling_tools.' 
    # in the method body.
    def render(self):
        """
        ...
        if Constants.DATA.value not in st.session_state:
        ...
        data = st.session_state[self.homepage_processor.DATA]
        ...
        st.session_state[Constants.DOCS.value] = st.session_state.get(Constants.DOCS.value, None)
        st.session_state[Constants.TOPIC_MODEL.value] = st.session_state.get(Constants.TOPIC_MODEL.value, None)
        st.session_state[Constants.TOPICS.value] = st.session_state.get(Constants.TOPICS.value, None)
        st.session_state[Constants.PROBS.value] = st.session_state.get(Constants.PROBS.value, None)
        st.session_state[Constants.LANGCHAIN_HISTORY.value] = st.session_state.get(Constants.LANGCHAIN_HISTORY.value, self.topic_modeling_tools.AGENTS)
        ...
        if st.sidebar.radio("Choose",["New model", "Load model"]) == "New model":
            ...
            if st.sidebar.button('Train'):
                with st.spinner('transforming documents...'):
                    docs, topic_model, embeddings, topics, probs, model_saved = self.topic_modeling_tools.fit_transform(
                    ...
                    st.session_state[Constants.DOCS.value] = docs
                    st.session_state[Constants.TOPIC_MODEL.value] = topic_model
                    st.session_state[Constants.EMBEDDINGS.value] = embeddings
                    st.session_state[Constants.TOPICS.value] = topics
                    st.session_state[Constants.PROBS.value] = probs
            ...
            if st.sidebar.button('Load'):
                with st.spinner('transforming documents...'):
                    docs, topic_model, embeddings, topics, probs = self.topic_modeling_tools.load_transform(_df=data, column=column, model_path=model_path, calculate_probs=calculate_probs, embedding_model_name=embedding_model_name)
                    st.session_state[Constants.DOCS.value] = docs
                    st.session_state[Constants.TOPIC_MODEL.value] = topic_model
                    st.session_state[Constants.EMBEDDINGS].value = embeddings
                    st.session_state[Constants.TOPICS.value] = topics
        ...
        if st.session_state[Constants.TOPIC_MODEL.value] is not None and st.session_state[Constants.TOPICS.value] is not None:
            self.topic_modeling_tools.display_data()
            with sample_empty.expander("Current state sample out of {}".format(len(data))):
                data = st.session_state[Constants.DATA.value]
...
if __name__ == "__main__":
    ...
    topic_modeling_tools = TopicModelingTools()
    topic_modeling_ui = TopicModelingUI(topic_modeling_tools, homepage_processor)
    topic_modeling_ui.render()
